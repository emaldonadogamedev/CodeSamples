...
bool GraphicsManager::InitLightConstantBuffer()
{
  ...
  m_pImmediateContext->PSSetConstantBuffers(1 /*register b1*/, 1, &m_pLightConstantBuffer);
...
{
  //Set Default values to Material
  //... TO DO
  ...
  m_pImmediateContext->PSSetConstantBuffers(2 /*register b2*/, 1, &m_pMaterialConstantBuffer);
...
void GraphicsManager::ResizeSwapChain()
{
  ...
  m_camera.m_Projection = XMMatrixPerspectiveFovLH(XM_PIDIV4, 
    m_aspectRatio = ( (float)m_ClientWidth / (float)m_ClientHeight ) , 0.01f, 1000.0f);
...
void GraphicsManager::UpdateLightCommonProperties(XMFLOAT4& cam_position, XMFLOAT4& global_ambient /*= XMFLOAT4(1,1,1,1)*/)
{
  m_lightConstBuffer.EyePosition = cam_position;

  //setting Ifog to clear buffer color
  m_lightConstBuffer.IFog = global_ambient;
}

void GraphicsManager::ToggleOnOffLight(int index)
{
}
...
bool GraphicsManager::Initialize()
{
  ...
  TextureManager::addTexture(L"metal_roof_spec_512x512.dds");// , true);
  TextureManager::addTexture(L"metal_roof_diff_512x512.dds");
  TextureManager::addTexture(L"TextureNormalTest.dds");// , true);
...
void GraphicsManager::Update(float dt)
{
  ...
  if (m_spinLights && m_spinningSpeed > 0)
  {
    ...
    //{
    //
    //  XMFLOAT4 temp = m_lightConstBuffer.Lights[i].m_Position;
    //  const float l = sqrt((temp.x * temp.x) + (temp.y * temp.y) + (temp.z * temp.z));
    //  float a = asin(temp.z / l);
    //  float a2 = ((a * 180.0f) / DirectX::XM_PI) + m_spinningSpeed;
    //  a2 = (a2 * DirectX::XM_PI) / 180.0f;
    //
    //  temp.x = temp.x * cos(a2) - temp.z * sin(a2);
    //  temp.z = temp.z * cos(a2) + temp.x * sin(a2);// *temp.z + temp.z;
    //  m_lightConstBuffer.Lights[i].m_Position = temp;
    //}
  }
...
void GraphicsManager::Render(float dt)
{
  ...
  UpdateLightCommonProperties(XMFLOAT4(m_camera.m_At.m128_f32));
    ...
    it != m_Components.end(); it++)
  {
    if ((*it)->m_IsActive)
    {
      ...
      XMFLOAT4 tempMin = reinterpret_cast<MeshRenderer*>(*it)->m_Mesh.m_minCoords;
      XMFLOAT4 tempMax = reinterpret_cast<MeshRenderer*>(*it)->m_Mesh.m_maxCoords;

      m_constBuffer.m_minCoords = tempMin;
      m_constBuffer.m_maxCoords = tempMax;
