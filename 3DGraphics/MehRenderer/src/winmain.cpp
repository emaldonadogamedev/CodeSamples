...
int isBinaryTreeSearch(TreeNode* tree)
{
  // the result variable
  bool result = true;
  ...
  if (tree)
  {
    ...
    if (tree->right)
    {
      ...
      if (tree->right->data > tree->data)
        //we use && to accumulate the results of all of the recursive function calls
        result = result && isBinaryTreeSearch(tree->right);
