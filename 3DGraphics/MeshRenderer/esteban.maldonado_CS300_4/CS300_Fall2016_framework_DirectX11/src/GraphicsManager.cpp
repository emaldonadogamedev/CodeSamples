...
continue_Init:
  ...
  //Bind viewport
  m_pImmediateContext->RSSetViewports(1, &m_mainViewport);
...
bool GraphicsManager::InitConstantBuffer()
{
  ...
  m_camera.m_Projection = XMMatrixPerspectiveFovLH(XM_PIDIV4, m_aspectRatio = ( (float)m_ClientWidth / (float)m_ClientHeight), 0.01f, 1200.0f);
...
void GraphicsManager::RenderSkyBox(float dt)
{
  m_constBuffer.mWorld = 
    XMMatrixTranspose( XMMatrixScaling(30, 30, 30) *
    XMMatrixTranslation(m_camera.m_Eye.m128_f32[0],
    m_camera.m_Eye.m128_f32[1],
    m_camera.m_Eye.m128_f32[2])
    );
  ...
  //cube map pass
  m_cubeMap->Render(dt, m_pDevice, m_d3dRasterStateDefault);
}
...
bool GraphicsManager::Initialize()
{
  ...
  //add cube map shaders...
  ShaderManager::addShader(ShaderType::VERTEX, "cubeMapVS");
  ShaderManager::addShader(ShaderType::PIXEL, "cubeMapPS");
  ...
  //Load basic shapes
  MeshData temp;
  MeshGenerator::CreateBox(temp, BOX_MESH);
  MeshGenerator::CreateBox(temp, SKY_BOX, 1, 1, 1);
  ...
  m_cubeMap->Initialize(m_pDevice);

  void* tempy = _aligned_malloc( sizeof(DynamicCubeMap), 16);
  m_dynCubeMap = new (tempy) DynamicCubeMap();
  m_dynCubeMap->Initialize(m_pDevice);
...
{
  //Render to texture working only on debug for now...
  m_dynCubeMap->RenderObjectsToTextures(m_pDevice, &m_Components, (MeshRenderer*)m_dynCubeMapMesh, m_constBuffer,
    m_pConstantBuffer);
  ...
  UpdateLightCommonProperties(XMFLOAT4(m_camera.m_Eye.m128_f32), XMFLOAT4(m_clearScreenColor.f));
...
void GraphicsManager::ShutDown()
{
  ...
  m_dynCubeMap->Release();
  _aligned_free(m_dynCubeMap);
