...
TestApp::TestApp(HINSTANCE hInstance) :DXApp(hInstance)
{
  ...
  m_graphics->m_dynCubeMapMesh = m_testMesh;
  ...
  //Prepare dynamic map objects...
  // 1 
  temp = _aligned_malloc(sizeof(IEntity), 16);
  m_testDynObj = new (temp)IEntity();
  m_testMesh2 = new MeshRenderer(m_testDynObj, "teapot.obj");
  m_testMesh2->m_phongMaterial->UseTexture = true;
  m_testMesh2->m_IsActive = true;
  m_testDynObj->m_Transform.m_Position.x = -5.0f;
  m_testDynObj->m_Transform.m_Scale.x = m_testDynObj->m_Transform.m_Scale.y = m_testDynObj->m_Transform.m_Scale.z = 0.02f;
  m_graphics->AddComponent(m_testMesh2);
  m_testDynObj->AddComponent(m_testMesh2);

  // 2
  temp = _aligned_malloc(sizeof(IEntity), 16);
  m_testDynObj2 = new (temp)IEntity();
  m_testMesh2 = new MeshRenderer(m_testDynObj2, "teapot.obj");
  m_testMesh2->m_phongMaterial->UseTexture = false;
  //m_testMesh2->m_phongMaterial->Emissive = { 0.000000000f, 1.000000000f, 1.000000000f, 1.000000000f };
  m_testMesh2->m_IsActive = true;
  m_testDynObj2->m_Transform.m_Position.x = 5.0f;
  m_testDynObj2->m_Transform.m_Scale.x = m_testDynObj2->m_Transform.m_Scale.y = m_testDynObj2->m_Transform.m_Scale.z = 0.02f;
  m_graphics->AddComponent(m_testMesh2);
  m_testDynObj2->AddComponent(m_testMesh2);
}

TestApp::~TestApp()
{
  //if you align alloc then you have to align free data
  _aligned_free(m_testG0);
  _aligned_free(m_testGO2);
  _aligned_free(m_testDynObj);
  _aligned_free(m_testDynObj2);
}
...
bool TestApp::Init()
{
  if ( DXApp::Init() )
  {
    ...
    TwAddVarRW(model_bar, "Use Reflection", TW_TYPE_BOOLCPP, &m_testMesh->m_phongMaterial->UseReflection, "group=Reflection");
    TwAddVarRW(model_bar, "Use Static Reflection", TW_TYPE_BOOLCPP, &m_testMesh->m_phongMaterial->UseStaticReflection, "group=Reflection");
    TwAddVarRW(model_bar, "Reclection Color", TW_TYPE_COLOR3F, &m_testMesh->m_phongMaterial->ReflectionColor, "group=Reflection");
    TwAddVarRW(model_bar, "Use Refraction", TW_TYPE_BOOLCPP, &m_testMesh->m_phongMaterial->UseRefraction, "group=Reflection");
...
void TestApp::Update(float dt)
{
  ...
  Transform* t = (Transform*)m_testDynObj->FindComponentByType(CT_TRANSFORM);
  t->m_Rotation.y += dt * 2.0f;

  t = (Transform*)m_testDynObj2->FindComponentByType(CT_TRANSFORM);
  t->m_Rotation.y += dt * 2.0f;
}
